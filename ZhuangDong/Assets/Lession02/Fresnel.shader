    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 frag (v2f i) : SV_TARGET 
            {
                ...
                float power3 = pow(1.0-max(0, dot(i.normalWS,viewDir)),3.0);
                color.rgb = saturate(1.0 - (1.0-color.rgb)*(1.0-power3 * _FresnelCol.rgb));

                //schlick公式
                float power5 = pow(1.0-max(0, dot(i.normalWS,viewDir)),5.0);
                color.rgb = saturate( color.rgb + (_FresnelCol.rgb - color.rgb)*power5);
                return float4(color);
            }
